...
for epoch in range(start_epoch, opt.niter+1):
    ...
    for i, data in enumerate(loader):
        ...
        if total_steps % opt.print_freq == print_delta:
            ...
            print('Total loss: %.5f; ganloss: %.5f; classloss: %.5f; '
                  'discriminatorloss: %.5f'
                  % (loss.data, ganloss.data, classloss.data, lossD.data))
        ...
        if save_fake:
            print('save imgs')
            print('')
            path = './result/syn/' + str(epoch) + '/' + str((i + 1) * opt.batch_size)
            util.mkdir(path)
            vutils.save_image(
    ...
    save_dir = opt.checkpoints_dir + '/TailorGAN_Garmentset/path/syn/'
    util.mkdir(save_dir)
