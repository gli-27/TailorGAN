# -*- coding: utf-8 -*-
"""
Created on Thu Mar 28 00:42:56 2019
...
class FashionDataset(Dataset):
    def __init__(self,params):
        info_file_path= './DATA/train_sleeve.csv' #params['train_anno_path']
    ...
    def __getitem__(self,index):
        ...
        img_name=row_df.imageName
        sleeve_type=row_df.type
        
        orig_imgPath= os.path.join('./DATA/Sleeve/highContra',img_name) #row_df.tgt_imgPath
        orig_img=io.imread(orig_imgPath)
        
        edge_imgPath= os.path.join('./DATA/Sleeve/edge/',img_name) #row_df.edge_imgPath
        edge_img =io.imread(edge_imgPath)
        edge_img=np.repeat(edge_img[:,:,np.newaxis], 3, axis=2)
        #print(edge_img.shape)
        src_imgPath=os.path.join('./DATA/Sleeve/crop/',img_name)
        src_img=io.imread(src_imgPath)
        ...
        return [edge_tensor, src_img_tensor, sleeve_type, orig_img_tensor]
