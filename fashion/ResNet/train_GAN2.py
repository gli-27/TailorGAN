...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(loader):
            ...
            # train discriminator 
            _, class_feat1=classifier2(gray_img.to(device))
            _, class_feat2=classifier2(prev_gray.to(device))
            rec_img =gen(src_img.to(device),class_feat1)
            fake_img=gen(src_img.to(device),class_feat2)
            for p in disc.parameters():
                p.requires_grad=True
            optimizer_D.zero_grad()
            #true_valid=disc(y.to(device).detach())
            #pred_valid=disc(fake_img.to(device).detach())
            true_valid, true_pred_class=disc(y.to(device).detach())
            pred_valid, fake_pred_class=disc(fake_img.to(device).detach())
            d_class=class_loss(true_pred_class,collar_type.to(device))
            d_loss=0.5*(adv_loss(true_valid,valid)+adv_loss(pred_valid,fake))+10.0*d_class#+10.0*class_loss(true_pred_class,collar_type.to(device))
            ...
            ## classifying loss
            #pred_class, _ =classifier1(fake_img.to(device))
            G_class_loss= class_loss(fake_pred_class,prev_type.to(device))
            G_rec_loss  = L1Loss(rec_img.to(device),y.to(device))
            ...
            print ('Epoch [{}/{}], Step [{}/{}], GLoss: {:.4f},G_adv: {:.4f}, G_class: {:.4f}, G_rec: {:.4f},DLoss:{:.4f},D_class: {:.4f}' .format(epoch+1, num_epochs, i+1,total_step,  g_loss.item(),G_adv_loss.item(), G_class_loss.item(),G_rec_loss.item(), d_loss.item(),d_class.item()))
            if i>0 and (i+1) % 4400==0:
