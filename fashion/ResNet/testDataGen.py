...
class testFashionDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        edge_img1 =io.imread(edge_imgPath1)
        edge_imgPath2=row_df2.edge_imgPath
        edge_img2 =io.imread(edge_imgPath2)
        ...
        part_edge_tensor1=self.transform(np.uint8(part_edge_img1))
        part_edge_tensor2=self.transform(np.uint8(part_edge_img2))
        p2p_tensor1 = self.p2pTransform(np.uint8(p2pImg1))
        p2p_tensor2 = self.p2pTransform(np.uint8(p2pImg2))
        mask1_tensor=self.masktransform(mask1.astype(np.uint8))
        mask2_tensor=self.masktransform(mask2.astype(np.uint8))
        collar_type= row_df2.collar_type
        
        return [orig_img_tensor1,orig_img_tensor2, src_img_tensor1,src_img_tensor2,mask1_tensor, mask2_tensor, part_edge_tensor1,part_edge_tensor2,edge_tensor1,edge_tensor2, p2p_tensor1, p2p_tensor2, collar_type]
