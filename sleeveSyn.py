...
for epoch in range(start_epoch, opt.niter+1):
    ...
    for i, data in enumerate(loader):
        ...
        refer_class_d, _, refer_feat_d = model.netD(refer_org_img.detach())
        ...
        if save_fake:
            print('save imgs')
            print('')
            path = './result/sleeveSyn/' + str(epoch) + '/' + str((i + 1) * opt.batch_size)
            util.mkdir(path)
            vutils.save_image(
    ...
    if epoch % 4 == 0:
        save_path_srcE = save_dir + 'FullModel_sleeve_srcE_%s.pth' % epoch
        torch.save(model.srcE.state_dict(), save_path_srcE)
        save_path_edgeE = save_dir + 'FullModel_sleeve_edgeE_%s.pth' % epoch
        torch.save(model.edgeE.state_dict(), save_path_edgeE)
        save_path_netG = save_dir + 'FullModel_sleeve_netG_%s.pth' % epoch
        torch.save(model.netG.state_dict(), save_path_netG)
        save_path_netD = save_dir + 'FullModel_sleeve_netD_%s.pth' % epoch
